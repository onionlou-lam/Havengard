...
public class PlayerMovement : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if (horizontal > 0)
		{
			GetComponent<Rigidbody2D>().linearVelocity = new Vector2(movementSpeed * Time.deltaTime, 0);
		...
		if (vertical > 0)
		{
			...
			GetComponent<Rigidbody2D>().linearVelocity =	new Vector2(GetComponent<Rigidbody2D>().linearVelocity.x, movementSpeed * Time.deltaTime);
		...
		if(isDead == true)
		{
			GetComponent<Rigidbody2D>().linearVelocity =	new Vector2(0.0f, 0.0f);
			animator.speed = 0.0f;
		}
